...
class WanTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        attn_output = self.attn1(hidden_states=norm_hidden_states, rotary_emb=rotary_emb, **attention_kwargs)
        hidden_states = (hidden_states.float() + attn_output * gate_msa).type_as(hidden_states)

        # 2. Cross-attention
        norm_hidden_states = self.norm2(hidden_states.float()).type_as(hidden_states)
        attn_output = self.attn2(hidden_states=norm_hidden_states, encoder_hidden_states=encoder_hidden_states, **attention_kwargs)
...
class WanTransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin):
    ...
    def forward(
        ...
        # fix rotary_emb for bbox_mask
        if attention_kwargs.pop('is_fixRope_step'):
            ...
            rotary_emb = rotary_emb.squeeze(0).squeeze(0).reshape(post_patch_num_frames, post_patch_height, post_patch_width, -1)
            bbox_mask = attention_kwargs.pop("bbox_mask")
            ...
            for frame_idx in range(0, bbox_mask.shape[0]):
                (fi_h1, fi_w1), (fi_h2, fi_w2) = find_corners(bbox_mask[frame_idx].squeeze(0))
                tmp = rotary_emb[frame_idx].clone()

                if f0_h2-f0_h1 == fi_h2-fi_h1 and f0_w2-f0_w1 == fi_w2-fi_w1:
                    # static bbox
                    rotary_emb[frame_idx, fi_h1:fi_h2, fi_w1:fi_w2] = tmp[f0_h1:f0_h2, f0_w1:f0_w2]
                else:
                    # dynamic bbox
                    tmp1 = tmp[f0_h1:f0_h2, f0_w1:f0_w2].permute(2, 0, 1)  # (c, h, w)
                    tmp2 = self.scale_complex_tensor(tmp1, (fi_h2-fi_h1, fi_w2-fi_w1))
                    tmp3 = tmp2.permute(1, 2, 0)  # (fi_h2-fi_h1, fi_w2-fi_w1, c)
                    rotary_emb[frame_idx, fi_h1:fi_h2, fi_w1:fi_w2] = tmp3

            rotary_emb = rotary_emb.reshape(1,1,post_patch_num_frames*post_patch_height*post_patch_width,-1)
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing:
            ...
            cnt = 0
            for block in self.blocks:
                global BLOCK_NAME, TIME_STEP
                BLOCK_NAME = f'block_{cnt}'
                TIME_STEP = int(timestep.data)
                cnt += 1
                if attention_kwargs is not None:
